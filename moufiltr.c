...
--*/   
{
	...
	ioQueueConfig.EvtIoRead = MouFilter_EvtIoRead;
...
)
{
	...
	currentIrp = WdfRequestWdmGetIrp(Request);
	MouseData = (PMOUSE_INPUT_DATA)currentIrp->AssociatedIrp.SystemBuffer;

	difX = devExt->LastX - MouseData->LastX;
	difY = devExt->LastY - MouseData->LastY;

	if (difX < 0) {
		DbgPrint("X move right.\n");
	}
	else if (difX > 0) {
		DbgPrint("X move left.\n");
	}

	if (difY < 0) {
		DbgPrint("Y move down.\n");
	}
	else if (difY > 0) {
		DbgPrint("Y move up.\n");
	}

	devExt->LastX = MouseData->LastX;
	devExt->LastY = MouseData->LastY;
